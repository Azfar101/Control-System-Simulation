# Ball and Beam Simulation with No Control System (Student-Friendly Template)
...
# Initial conditions
initial_state = [L / 2, 0.0]  # Ball starts at the center of the beam with zero velocity
...
disturbance_start = 1.0  # Time when disturbance starts (seconds)
disturbance_duration = 3.0  # Duration of the disturbance (seconds)
disturbance_force = 0.2  # Magnitude of the wind-like disturbance

# Placeholder for a control system
def control_system(state, t):
    """
    Placeholder for a user-defined control system.
    
    Parameters:
    - state: List [x, x_dot], where x is the ball's position, x_dot is the ball's velocity.
    - t: Current time in seconds.
    
    Returns:
    - u: Beam angle in radians, where u ∈ [-0.2, 0.2].
    """
    x, x_dot = state
    # Example: A simple placeholder for a control system (students can modify this)
    u = 0.0  # Default: No control
    return np.clip(u, -0.2, 0.2)

# State-space model with wind disturbance
def ball_and_beam_dynamic(t, state):
    x, x_dot = state
    # Call the user-defined control system
    u = control_system(state, t)
...
ax.set_title("Ball and Beam System (No Control System)")
